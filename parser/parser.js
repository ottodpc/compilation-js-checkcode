...
module.exports=(tokens) =>{
    ...
    for(let i= 0; i<tokens.length; i++){
        ...
        if(tokens[i].type && tokens[i].type == constTokens.typeWord && constParser.declarationVariable.indexOf(tokens[i].value)!=-1){
            expression= factory.create(constParser.expressionDeclaration, tokens, i);
            i++;
        //utilisation symbole égale
        }else if(tokens[i].type && tokens[i].type  == constTokens.symboleEqual){
            expression= factory.create(constParser.expressionAffectation, tokens, i);
            //si affectation nombre
            if(expression.variableValue.type && expression.variableValue.type== constTokens.typeNumber){
                i++;
        ...
        }else if(i<tokens.length-1 && tokens[i].type && tokens[i].type  == constTokens.typeWord &&  tokens[i+1].type==constTokens.symbolePoint){
